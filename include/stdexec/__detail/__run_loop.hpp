...
namespace STDEXEC
{
  ...
  namespace __run_loop
  {
    class __run_loop_base
    {
     public:
      ...
      struct __opstate_t : __task
      {
        ...
        STDEXEC_ATTRIBUTE(host, device)
        constexpr explicit __opstate_t(__std::atomic<std::size_t>*                 __task_count,
    ...
    template <class _Derived, class _Env>
    struct __basic_run_loop : __run_loop_base
    {
     private:
      friend _Derived;
      ...
      struct __attrs_t
      {
        ...
        STDEXEC_ATTRIBUTE(nodiscard, host, device)
        constexpr auto query(__execute_may_block_caller_t) const noexcept
        {
          return false;
        }

        __basic_run_loop* __loop_;
      };

      class __scheduler : __attrs_t
      {
       private:
        friend __basic_run_loop;
        friend struct _Derived::scheduler;

        STDEXEC_ATTRIBUTE(host, device)
        constexpr explicit __scheduler(__basic_run_loop* __loop) noexcept
          : __attrs_t{__loop}
        {}
       ...
       public:
        ...
        struct __sndr_t
        {
          ...
          {
            return __opstate_t<_Rcvr>{&__loop_->__task_count_,
                                      &__loop_->__queue_,
         ...
         private:
          friend __scheduler;
          STDEXEC_ATTRIBUTE(host, device)
          constexpr explicit __sndr_t(__basic_run_loop* __loop) noexcept
            : __loop_(__loop)
          {}
        ...
        STDEXEC_ATTRIBUTE(nodiscard, host, device)
        friend constexpr bool operator==(__scheduler const & __a, __scheduler const & __b) noexcept
        {
      ...
      __basic_run_loop() = default;

      STDEXEC_ATTRIBUTE(host, device)
      constexpr explicit __basic_run_loop(_Env __env) noexcept
     ...
     public:
      ...
      {
        return typename _Derived::scheduler{this};
      }
  ...
  struct run_loop : __run_loop::__basic_run_loop<run_loop, env<>>
  {
    ...
    struct scheduler : __scheduler
    {
     private:
      ...
      STDEXEC_ATTRIBUTE(host, device)
      constexpr explicit scheduler(__basic_run_loop* __loop) noexcept
        : __scheduler{__loop}
      {}
    };
  };
    ...
    get_completion_scheduler_t<set_value_t>) const noexcept
  {
    ...
    {
      return typename _Derived::scheduler{__loop_};
    }
  }
